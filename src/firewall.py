...
def analyse_datagram(datagram_header):
    ...
    saddress = ipaddress.ip_address(int('.'.join(str(int(x, 2)) for x in saddress_bin.split())))
    daddress = ipaddress.ip_address(int('.'.join(str(int(x, 2)) for x in daddress_bin.split())))
    sport = int(payload[:16],2)
    dport = int(payload[16:32],2)
    headers = {'saddress': saddress, 'daddress': daddress, 'sport': sport, 'dport': dport, 'protocol': protocol}
    return (headers)
